...
第六天进度:
    主要工作:搜索框功能的实现,添加商品至购物车,以及购物车页面的显示

    1.实现搜索框
        利用haystack全文检索框架和纯python实现的whoosh搜索引擎,并利用jieba对其进行扩展

        1)配置haystack到django中,将需要检索的数据库字段添加至制定txt文件
        2)配置url search 当请求提交至search时haystack会帮我们跳转至search.html
        3)使用检索后取出的内容渲染模板

    2.添加物品至购物车
        我们购物车中的信息利用redis进行存储,这里使用ajax来实现
        
        1)当用户点击加入购物车时通过ajax向服务器发起请求,并将商品的id已经要购买的数量传递过去
        2)通过django视图接收ajax传递的参数,将其存入redis中
        

    3.显示购物车页面
        从redis中读取数据通过上下文传递给模板进行显示
        ...
        4)利用js实现部分功能
            (4.1)当用户点击全选时,勾选所有商品
            (4.2)当用户取消全选项时取消所有勾选
            (4.3)当用户先点击全选,再取消其中某个商品时,全选框取消选择
            (4.4)根据勾选动态生成商品数量,及总价格

第七天进度:
    主要工作:购物车功能完善,订单的生成.
    
    1.购物车功能扩展
        利用ajax的post请求实现前端页面与视图页面的交互

        1)当用户点击+ - 号时,redis中存储购物车的数据库随之更新,并返回更新成功或失败的标记
        2)当返回更新失败的标记时弹出对应错误原因,更新成功时对应的前端页面数据进行更新
        
    2.订单生成
        创建一条订单,即在数据库的订单表中增加一条数据,在订单商品表中增加商品sku种类的数量

        1)当用点击提交订单时,发起ajax post请求,传递给视图,商品id 以及支付方式,收货地址信息.
